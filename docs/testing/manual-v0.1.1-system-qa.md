...
| --- | --- | --- | --- |
| First-run onboarding | Clean preferences, launch app, step through all eight pages, click Open Menu Bar Settings on the native cleanup page, return to onboarding, complete it, relaunch | Onboarding includes the Apple Menu Bar settings cleanup step, the Arrange test step, opens System Settings best-effort, persists completion, and triggers no permission prompt | Page title, opened System Settings destination, whether completion persisted, diagnostics |
...
## Phase 14 Manual QA Additions

| Area | Steps | Expected Result | Capture On Failure |
| --- | --- | --- | --- |
| Arrange manual QA | Open Settings -> Arrange, follow the Command-drag guide, move the control item manually, move the primary separator manually, test collapse, test reveal all, test reset layout, then enable always-hidden zone and repeat the placement check | Guided Manual Arrange is understandable, no Pro or Accessibility prompt appears, Basic controls remain reachable, and reset returns to a usable expanded state | Display setup, menu bar auto-hide state, exact step, diagnostics |
| Placement Planner manual QA | Enable Pro Mode, enable Accessibility Discovery, grant Accessibility only from the explicit permission button, run or wait for a scan, open Arrange, review recommendations | Planner shows recommendations when scan data is usable, shows clear degraded states when gates are missing, does not move anything, and diagnostics do not include raw item names by default | Gate state, scan age, recommendation count, diagnostics |
| Assisted Move manual QA | Enable Experimental Icon Moving, choose one non-system third-party item, run dry-run, cancel confirmation, confirm no move happened, then run one confirmed move if safe to test | Assisted Move remains single-item and Experimental, cancel prevents movement, success verifies target or failure offers reveal/reset/diagnostics, and system items are blocked by default | Target item description, target zone, before/after notes, diagnostics |
| New Item Inbox manual QA | Launch fixture app with a new item, enable Pro Discovery gates, run or wait for a scan, then confirm the New Items count appears in Find & Rescue and the status menu shows Open New Items. Use current unit/store tooling for dismissal/reset until the dedicated review-list UI ships. | New items are detected only behind Pro Discovery, the status menu entry appears only when the review count is nonzero and Pro Discovery gates are satisfied, and diagnostics avoid raw item identity by default | Fixture item count, status-menu title, diagnostics |
