...
| Shortcuts/App Intents | Discover actions in Shortcuts, run stable actions, run blocked states with automation paused/Safe Mode/Labs off/Pro off | Safe Mode and pause gates block automation; Labs and Pro gates fail closed; blocked action reports a clear reason | Shortcut/action name, gate state, returned result, diagnostics |
| URL automation | Exercise `menubardeclutter://` routes for safe actions and blocked actions, including automation paused and Safe Mode | URL commands do not bypass Safe Mode, automation pause, Pro gates, Labs gates, or Private Access gates | URL command, gate state, returned result, diagnostics |
...
| Import/Export Preview | Export a settings package, inspect JSON metadata, choose package for import dry-run, verify no apply button appears | Export uses real values plus omission metadata; import reports dry-run/backup only and does not mutate settings | Package metadata, dry-run counts, backup path, diagnostics |
