...
| Compact strip screenshot QA | PASS | `scripts/qa_capture_ui_screenshots.sh --build --focused-only --output-dir docs/testing/screenshot-qa/2026-07-06_secondbar-compact-fallback-icons` captured 21 surfaces with 0 skipped and 0 failed. Manifest rows `32-compact-second-bar` and `33-compact-second-bar-fallback-icons` each captured a `166x42` `Second Bar Compact Strip` window. The fallback row used full readiness gates, primary-click opt-in, seeded menu bar items, and only a partial rendered icon seed so a missing thumbnail stayed in the one-line strip with fallback icon rendering. |
