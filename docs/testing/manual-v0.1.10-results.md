...
- Pro Second Bar direct activation matrix helper: PASS. `scripts/qa_second_bar_activation_matrix.sh` converts sanitized diagnostics JSON activation logs into markdown matrix rows; `secondBarActivationMatrixScriptExtractsDiagnosticsRows` covers field extraction and ignores unrelated logs.
...
- Pro Second Bar compact fallback icons: PASS. `xcodebuild build-for-testing -scheme MenuBarDeclutter -destination 'platform=macOS,arch=arm64' -derivedDataPath build/DerivedData/secondbar-compact-fallback-icons ENABLE_DEBUG_DYLIB=YES -quiet` passed, and direct `xcrun xctest` on `MenuBarDeclutterTests.xctest` passed 586 tests in 77 suites. Coverage includes Hidden-zone items staying visible in the compact strip even when a specific rendered thumbnail is missing, and `+N` counting only one-line overflow.
- Pro Second Bar compact live diagnostics: PASS. `xcodebuild build-for-testing -scheme MenuBarDeclutter -destination 'platform=macOS,arch=arm64' -derivedDataPath build/DerivedData/secondbar-compact-live-diagnostics ENABLE_DEBUG_DYLIB=YES -quiet` passed after adding the Foundation test import, and direct `xcrun xctest` on `MenuBarDeclutterTests.xctest` passed 587 tests in 77 suites. Coverage includes privacy-safe live diagnostics for last compact visible count, overflow count, fallback-icon count, and scan state.
- Pro Second Bar runtime diagnostics export: PASS. `xcodebuild build-for-testing -scheme MenuBarDeclutter -destination 'platform=macOS,arch=arm64' -derivedDataPath build/DerivedData/secondbar-compact-notch-diagnostics ENABLE_DEBUG_DYLIB=YES -quiet` passed, and `xcodebuild test-without-building -scheme MenuBarDeclutter -destination 'platform=macOS,arch=arm64' -derivedDataPath build/DerivedData/secondbar-compact-notch-diagnostics -only-testing:MenuBarDeclutterTests -quiet` passed. Coverage includes privacy-safe JSON/text export of Second Bar visible state, item count, warm-up state, last compact visible count, overflow count, fallback-icon count, scan state, and notch-avoidance placement state.
...
- Pro Second Bar compact screenshot QA: PASS. `scripts/qa_capture_ui_screenshots.sh --build --focused-only --output-dir docs/testing/screenshot-qa/2026-07-06_secondbar-requirements-states-final2` passed with 24 captured surfaces, 0 skipped, and 0 failed. The run captured `32-compact-second-bar` and `33-compact-second-bar-fallback-icons` as `166x42` ready/fallback `Second Bar Compact Strip` windows, plus `34-compact-second-bar-accessibility-required`, `35-compact-second-bar-accurate-icons-required`, and `36-compact-second-bar-screen-recording-required` as compact requirements strips for revoked Accessibility, missing Accurate Icons, and missing Screen Recording gates.
- Pro Second Bar warm-up diagnostics: PASS. `xcodebuild build-for-testing -scheme MenuBarDeclutter -destination 'platform=macOS,arch=arm64' -derivedDataPath build/DerivedData/secondbar-warmup-diagnostics ENABLE_DEBUG_DYLIB=YES -quiet` passed, and direct `xcrun xctest` on `MenuBarDeclutterTests.xctest` passed 584 tests in 77 suites. Coverage includes privacy-safe live diagnostics for Second Bar icon warm-up in-progress and refreshed-count reporting.
- Pro Second Bar readiness diagnostics export: PASS. `xcodebuild build-for-testing -scheme MenuBarDeclutter -destination 'platform=macOS,arch=arm64' -derivedDataPath build/DerivedData/secondbar-readiness-diagnostics ENABLE_DEBUG_DYLIB=YES -quiet` passed, and direct `xcrun xctest` on `MenuBarDeclutterTests.xctest` passed 585 tests in 77 suites. Coverage includes privacy-safe JSON/text export of Second Bar readiness state, entitlement, Accessibility Discovery, Accessibility permission, Accurate Icons, Screen Recording, primary-click route, and Safe Mode state.
- Canonical app build after the compact requirements screenshot QA update: PASS, `xcodebuild -scheme MenuBarDeclutter -destination 'platform=macOS' build -quiet` exited 0 with only the expected multiple-destination warning.
