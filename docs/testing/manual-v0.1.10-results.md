...
- Pro Second Bar compact screenshot QA: PASS. `scripts/qa_capture_ui_screenshots.sh --build --focused-only --output-dir docs/testing/screenshot-qa/2026-07-06_secondbar-compact-fallback-icons` passed with 21 captured surfaces, 0 skipped, and 0 failed. The run captured `32-compact-second-bar` and `33-compact-second-bar-fallback-icons` as `166x42` `Second Bar Compact Strip` windows after updating the screenshot harness for the full readiness gate, one-line compact strip height, and partial rendered-icon fallback path.
...
- Canonical app build after the compact fallback screenshot QA update: PASS, `xcodebuild -scheme MenuBarDeclutter -destination 'platform=macOS' build -quiet` exited 0 with only the expected multiple-destination warning.
