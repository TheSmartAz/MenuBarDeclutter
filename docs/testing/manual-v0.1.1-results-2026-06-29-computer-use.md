...
| Automation/App Intents Settings gates | PASS | Computer Use verified preview action labels: local Basic Mode actions `Ready`, `Apply Profile` `Profile Gate`, and `Set Layout Spacing Preset` `Labs Gate`. Toggling in-app gates changed labels to `Ready`, `Requires Labs`, and `Disabled` as expected, then settings were restored. |
